...
def create_demo():
    with gr.Blocks() as demo:
        with gr.Row():
            prompt = gr.Textbox(lines=4, label="Synthesis Procedure")
        with gr.Row():
            run_botton = gr.Button("Run", variant='primary')
        with gr.Row():
            gr.Markdown("ORD JSON")
        with gr.Row():
            json_output = gr.JSON(label='JSON Out')
...
if dist.get_rank() == 0:
    description = f"""
    # Convert synthesis procedure to ORD JSON
    Model loaded from: {args.pretrained_path}
    """
